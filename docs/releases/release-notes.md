...
### 2026-08-16（短剧视频通道、自动导演恢复与漫画人物稿）

- 短剧分镜视频可以接入 OpenAI 风格的异步 Videos API，支持提交画面比例、时长和清晰度，并在任务完成后读取可播放的视频结果；可用于 CLI Proxy 暴露的 Grok 视频模型。
- 自动导演在书级方向生成阶段遇到模型连接中断后，可以从原任务继续生成候选；旧任务未保存预计章节数时，确认方向会采用候选方案的章节规模继续，恢复成功后也会清除旧错误。
- 漫画内容源重复导入时会保留已有角色及其三视图、表情稿和视觉资产；人物图片仍在生成时会阻止重导并给出可操作提示，避免生成完成后因角色被替换而保存失败。
