...
- 短剧分镜视频可以接入 OpenAI 风格的异步 Videos API，支持 CLI Proxy 暴露的 Grok 视频模型，并在生成完成后返回可播放结果。
- 自动导演生成书级方向时若遇到模型连接中断，可以从原任务继续；旧任务未保存预计章节数时，确认方向会采用候选方案的章节规模继续，恢复成功后也会清除旧错误。
- 漫画内容源重复导入时会保留已有角色和人物稿；图片生成期间会阻止重导，避免人物稿生成成功却因角色被替换而保存失败。
